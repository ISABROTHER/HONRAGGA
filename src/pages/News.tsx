...
function ContributionConfirmModal({
  ...
  const formatCurrencyModal = (n: number): string => {
      if (!n || n <= 0) return '...';
      try {
          return new Intl.NumberFormat(userLocale, { style: 'currency', currency: currency, maximumFractionDigits: 0 }).format(Math.round(n || 0));
      } catch {
                ...
                <h2 id="donate-form-heading" className="sr-only">Donation Form</h2>
                 ...
                 <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-5 md:p-8 space-y-6">
                    ...
                    {/* Section 3: Payment Method - Reduced size */}
                     <div>
                        {/* ... (Payment Method unchanged styling) ... */}
                         <p className="text-base font-bold text-green-900 mb-2">3. Choose Payment Method</p><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setPaymentMethod('paystack')} aria-pressed={paymentMethod === 'paystack'} className={['w-full rounded-md px-3 py-2 border text-sm font-medium transition', paymentMethod === 'paystack' ? 'bg-green-800 text-white border-green-800 ring-1 ring-green-900' : 'bg-white text-gray-700 border-gray-300 hover:bg-green-50 hover:border-green-600',].join(' ')}> Paystack </button>{currency === 'USD' ? (<button type="button" onClick={() => setPaymentMethod('applePay')} aria-pressed={paymentMethod === 'applePay'} className={['w-full rounded-md px-3 py-2 border text-sm font-medium transition', paymentMethod === 'applePay' ? 'bg-black text-white border-black ring-1 ring-gray-900' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100 hover:border-black',].join(' ')}>  Apple&nbsp;Pay </button>) : (<div className="w-full rounded-md px-3 py-2 border border-dashed border-gray-300 text-gray-400 text-center text-xs"> Apple&nbsp;Pay (USD only)</div>)}</div><p className="mt-2 text-xs text-gray-500">Using Visa/Mastercard? Select Paystack. For GHS, Paystack (MoMo/Card) is required.</p>
                    </div>
                    ...
                    <div>
                         <div className="text-center text-xs text-gray-500">
                             Secure payments via <span className="font-semibold">Paystack</span> & <span className="font-semibold">Apple Pay</span> supported. SSL secured & privacy-first.
                         </div>
                    </div>
