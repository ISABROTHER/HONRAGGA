...
import { MessageSquareWarning, HardHat, Users, Vote, HandHeart, UserCircle } from "lucide-react";
...
const quickLinks: QuickLink[] = [
  ...
  {
    title: "Assembly Members", // REPLACED POLICIES
    ...
    route: "assemblymen" // CORRECTED ROUTE
  },
  {
    title: "Polls & Tracker",
...
export function QuickAccessGrid({ onNavigate }: { onNavigate: (page: string) => void }) {
  return (
      ...
      <div className="max-w-[95%] 2xl:max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8">
        <div className="text-center mb-8 md:mb-14">
          ...
          <div className="mt-4 flex flex-col items-center justify-center group">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight text-center bg-gradient-to-r from-slate-900 via-green-700 to-slate-900 bg-clip-text text-transparent">
              Information, Services & Support
            </h2>
            <span className="mt-3 h-1 w-16 rounded-full bg-gradient-to-r from-green-500 via-emerald-500 to-green-600 group-hover:w-32 transition-all duration-500" />
          </div>
          <p className="text-base md:text-lg text-slate-600 mt-5 max-w-3xl mx-auto leading-relaxed">All the key information you need about the constituency is organised here for easy access.</p>
        </div>
        <div className="grid grid-cols-3 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-8 lg:gap-10">
          {quickLinks.map((link, idx) => (
            <button key={idx} onClick={() => onNavigate(link.route)} className={`group relative overflow-hidden rounded-xl md:rounded-3xl border ${link.bgClass} bg-gradient-to-br p-3 md:p-10 flex flex-col items-center text-center cursor-pointer hover:shadow-xl hover:-translate-y-2 transition-all`}>
              <div className="w-12 h-12 md:w-20 md:h-20 bg-white rounded-xl md:rounded-3xl flex items-center justify-center shadow-sm mb-2 md:mb-6 group-hover:scale-110 transition-transform"><link.icon className={`w-6 h-6 md:w-10 md:h-10 ${link.iconColor}`} /></div>
